...
const visibleColumns = computed(() => {
	if (!props.advanced) return props.columns
...
const toggleSort = (col, event) => {
	if (!props.advanced || col.sortable === false) return
	const key = col.key
...
const isAllSelected = computed(() => {
	...
	return displayData.value.every((row) => selectedRows.value.includes(row.id))
})
...
const toggleSelectAll = () => {
	if (isAllSelected.value) {
		const currentIds = displayData.value.map((r) => r.id)
		selectedRows.value = selectedRows.value.filter((id) => !currentIds.includes(id))
	} else {
		const currentIds = displayData.value.map((r) => r.id)
		const newSelected = Array.from(new Set([...selectedRows.value, ...currentIds]))
...
const filteredData = computed(() => {
	if (!props.advanced) return props.data
...
<template>
	<div v-if="!advanced" class="overflow-x-auto">
		<table :class="tableClasses">
			<thead class="table-thead">
				<tr>
					<th v-if="selectable" class="table-th w-10 text-center px-3">
			...
			<tbody>
				<tr v-for="(row, rowIndex) in data" :key="row.id || rowIndex" :class="hoverable ? 'table-row' : ''">
					<td v-if="selectable" class="table-td text-center px-3">
						<div class="flex items-center justify-center">
				...
				<tr v-if="data.length === 0">
					<td :colspan="columns.length + (selectable ? 1 : 0)" class="table-td text-center py-8 text-muted-foreground text-xs">
						<slot name="empty">
							No data available
						</slot>
					</td>
			...
			<div class="flex items-center gap-2 flex-1 max-w-md">
				<div class="w-full">
					<BaseInput v-model="searchInput" type="text" @input="handleSearchInput" placeholder="Search table records...">
						...
						<template #suffix>
							<button v-if="searchInput" @click="searchInput = ''; debouncedSearch = ''; currentPage = 1" class="text-muted-foreground hover:text-foreground cursor-pointer">
								<IconX :size="14" />
							</button>
						</template>
					</BaseInput>
			...
			<div class="overflow-x-auto overflow-y-auto" :style="{ maxHeight: stickyHeader ? maxHeight : 'none' }">
				<table :class="tableClasses">
					...
					<tbody>
						...
						<tr v-else v-for="(row, rowIndex) in displayData" :key="row.id || rowIndex" :class="[
